    ...
    def get_baseprice_volume(self):
        try:
            ...
            if self.FlwExchange == 'Okex':
                ...
                return (float(now_price) * (1 + self.FlwMul * t_rate)), volume
        ...
        while True:
            try:
                ...
                if side == TakingSide.UP and R != [] and L != []:
                    ...
                    # 铺卖单
                    ret = self.submit_orders(side=TradeSide.SELL, orders=R, delay=1.5)
                    time.sleep(0.5)
                    # 下反手买单
                    ret = self.submit_orders(side=TradeSide.BUY, orders=L, delay=self.kline_delay)
                    # 反手单加量的时候，撤单
                    ...
                    # 铺买单
                    ret = self.submit_orders(side=TradeSide.BUY, orders=R, delay=1.5)
                    time.sleep(0.5)

                    # 下反手卖单
                    ret = self.submit_orders(side=TradeSide.SELL, orders=L, delay=self.kline_delay)
